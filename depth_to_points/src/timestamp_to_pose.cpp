...
bool Gps2Pose::compareTimestamp(const std::string& timestamp_1, const std::string& timestamp_2) {
    ...
    else if (first_part_1 > first_part_2) return false;
...
void Gps2Pose::timestamp2Pose(const std::string& timestamp, Eigen::Vector3d& pos, Eigen::Quaterniond& q_inv) {
    GpsData gps_data;
    timestamp2Gps(timestamp, gps_data);
    gps2Pose(gps_data, pos, q_inv);
}